...
class PakasirService
{
     ...
     */
    public static function markPaid(Payment $payment, bool $isSimulated = false): void
    {
        DB::transaction(function () use ($payment, $isSimulated): void {
            ...
            if ($isSimulated) {
                VoucherService::release($lockedPayment);
            } else {
                VoucherService::consume($lockedPayment);
            }

            if ($lockedPayment->session) {
                $timerSetting = TimerSetting::resolveForCafe($lockedPayment->session->cafe_id);
                $duration = $timerSetting->session_timeout_seconds ?? 300;
                $lockedPayment->session->update([
                    'status' => 'active',
                    'started_at' => now(),
